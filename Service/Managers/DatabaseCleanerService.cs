...
namespace NORCE.Drilling.EarthMagneticField.Service.Managers
{
    public class DatabaseCleanerService : BackgroundService
    {
        ...
        private readonly string[] _dataTables = ["EarthMagneticFieldCalculationOrderTable"];
        ...
        /// </summary>
        /// <returns>true if older EarthMagneticField were successfully deleted</returns>
        private bool CleanDatabase(string dataTable)
        {
            ...
            if (connection != null)
            {
                ...
                {
                    _logger.LogError(ex, "Impossible to clean old EarthMagneticField from {table}", dataTable);
                }
            }
            else
            {
